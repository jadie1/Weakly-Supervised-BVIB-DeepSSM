...
class BayesianDeepSSMNet(nn.Module):
	def __init__(self, config_file, dataset):
		...
		# Set dimensions
		self.y_dim = dataset.corrs[0].shape[0]*3
		self.x_dim = dataset.imgs.shape[-3:]
		...
		# Set decoder
		if config['decoder']['linear']:
			self.decoder = LinearDecoder(self.z_dim, self.y_dim)
		else:
			if config['decoder']['stochastic']:
				self.decoder = NonLinearDecoder(self.z_dim, self.y_dim, self.dropout, self.batch_ensemble)
			else:
				self.decoder = NonLinearDecoder(self.z_dim, self.y_dim)
...
class ConvolutionalBackbone(nn.Module):
    def __init__(self, x_dim, dropout={"type":None}, batch_ensemble={"enabled":False}):
        ...
        self.conv_blocks.append(Conv3d_Block( 1, 12, 5, batch_norm=BATCH_NORM, max_pool=True,  be_num_models=self.batch_ensemble_num_models, be_mixup=self.mixup))
        self.conv_blocks.append(Conv3d_Block(12, 24, 5, batch_norm=BATCH_NORM, max_pool=False, be_num_models=self.batch_ensemble_num_models, be_mixup=self.mixup))
        self.conv_blocks.append(Conv3d_Block(24, 48, 5, batch_norm=BATCH_NORM, max_pool=True,  be_num_models=self.batch_ensemble_num_models, be_mixup=self.mixup))
        self.conv_blocks.append(Conv3d_Block(48, 96, 5, batch_norm=BATCH_NORM, max_pool=False, be_num_models=self.batch_ensemble_num_models, be_mixup=self.mixup))
        self.conv_blocks.append(Conv3d_Block(96,192, 5, batch_norm=BATCH_NORM, max_pool=True,  be_num_models=self.batch_ensemble_num_models, be_mixup=self.mixup))
...
class LinearDecoder(nn.Module):
	def __init__(self, num_latent, num_corr):
		...
		self.pred_y_mean = nn.Linear(self.num_latent, self.num_corr)
	def forward(self, z, use_dropout=False):
		y_mean =  self.pred_y_mean(z)
		# y_mean = y_mean.reshape(*y_mean.size()[:-1], self.num_corr//3, 3)
		regularization = torch.tensor(0, device=z.device).type(z.dtype) # placeholder		
		return y_mean, regularization
